...
void NetCore::ProcessPacket(char* packet_buf) {
	...
	case LC_MATCHSTART: {
		...
		m_room_no = p->roomNum;
		m_ishost = p->is_host;
		Service::GetApp()->Notify(EVENT_LOBBY_MATCH_OK, 2, m_room_no, m_ishost);
		break;
	}
	case LC_CANCEL_MATCH_OK: {
	...
	case BC_ACCEPT_OK: {
		...
		Service::GetApp()->Notify(EVENT_BATTLE_ACCEPT_OK, 2, m_room_no, m_ishost);
		break;
	}
	case BC_ACCEPT_FAIL: {
		cout << "ACCEPT FAIL\n";
	...
	case BC_UPDATED_USER_INFO: {
		...
		Service::GetApp()->Notify(EVENT_LOBBY_UPDATE_SERVER_USERINFO, 1, p->mmr);
		break;
	}
	default: {

...
void CScene::ReleaseObjects()
{
	...
	if (m_pObjectShaders)
	{
		for (int i = 0; i < m_nObjectShaders; i++) m_pObjectShaders[i].ReleaseShaderVariables();
		for (int i = 0; i < m_nObjectShaders; i++) m_pObjectShaders[i].ReleaseObjects();
		delete[] m_pObjectShaders;
	}
...
bool CScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		{
		case VK_F4:
			m_pLights->m_pLights[1].m_bEnable = !(m_pLights->m_pLights[1].m_bEnable);
			break;
		default:
			break;
		}

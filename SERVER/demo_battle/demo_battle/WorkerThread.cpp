...
message WorkerThread::ProcPacket(int id, void* buf) {
	...
	msg.id = id;
	msg.type = PacketType::NO_MSG;
	msg.anim_type = -1;

#ifdef LOG_ON
	std::cout << "[WORKER] ProcPacket Type: " << (int)inputPacket[1] << " ID: " << id << endl;
#endif //LOG_ON
	switch (static_cast<PacketType>(inputPacket[1])) {
	case PacketType::LB_REQUEST_ROOM: {
		int roomNo;
	...
	case PacketType::CB_MAKE_ANIM: {
		...
		msg.id = p->id;
		msg.type = PacketType::CB_MAKE_ANIM;
	...
	case PacketType::CB_POSITION_VECTOR: {
		...
		msg.id = p->id;
		msg.type = PacketType::CB_POSITION_VECTOR;
		msg.vec.x = p->pos.x;
		msg.vec.y = p->pos.y;
		msg.vec.z = p->pos.z;
		msg.anim_type = p->anim_type;
	...
	case PacketType::CB_LOOK_VECTOR: {
		...
		msg.id = p->id;
		msg.type = PacketType::CB_LOOK_VECTOR;
		msg.vec.x = p->look.x;
		msg.vec.y = p->look.y;
		msg.vec.z = p->look.z;
		break;

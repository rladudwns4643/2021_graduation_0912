...
void Service::ActiveService() {
	NetCore::GetApp()->CreateSocket(eSERVER::LOBBY);
	NetCore::GetApp()->CreateSocket(eSERVER::BATTLE);
	NetCore::GetApp()->ConnectServer(eSERVER::LOBBY);
...
void Service::AddEvent(int sEvent, int argsCount, ...) {
	...
	case EVENT_LOBBY_CONNECT_FAIL: {
		lobby_active = false;
		NetCore::GetApp()->DisconnectServer(eSERVER::LOBBY);
		break;
	}
	...
	case EVENT_LOBBY_REQUEST_FIND_ROOM: {
		NetCore::GetApp()->ConnectServer(eSERVER::BATTLE);

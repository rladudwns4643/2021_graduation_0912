...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	if (dwDirection)
	{
		...
		// 화살표 키 ‘↑’를 누르면 로컬 z-축 방향으로 이동(전진)한다. ‘↓’를 누르면 반대 방향으로 이동
		if (dwDirection & DIR_FORWARD) xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Look, fDistance);
		if (dwDirection & DIR_BACKWARD) xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Look, -fDistance);

		// 화살표 키 ‘→’를 누르면 로컬 x-축 방향으로 이동한다. ‘←’를 누르면 반대 방향으로 이동
...
CTestPlayer::CTestPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int nMeshes) : CPlayer(nMeshes)
{
	// 지형
	...
	XMFLOAT3 SetPPos(pTerrain->GetWidth() * 0.5f, fHeight + 500.0f, pTerrain->GetLength() * 0.5f);
	SetPosition(SetPPos);
	// 플레이어 속도를 설정
	SetSpeed(200.f);
...
CCamera* CTestPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	{
	case FIRST_PERSON_CAMERA:
		// 플레이어의 특성을 1인칭 카메라 모드에 맞게 변경(중력은 적용X)
		SetFriction(250.0f);
		SetPGrav = XMFLOAT3(0.0f, -250.0f, 0.0f);
		SetGravity(SetPGrav);
		SetMaxVelocityXZ(300.0f);
		SetMaxVelocityY(400.0f);
		m_pCamera = OnChangeCamera(FIRST_PERSON_CAMERA, nCurrentCameraMode);
		m_pCamera->SetTimeLag(0.0f);
		m_pCamera->SetOffset(XMFLOAT3(0.0f, STD_CUBE_SIZE / 2.0f, 0.0f));
		m_pCamera->GenerateProjectionMatrix(1.01f, 5000.0f, ASPECT_RATIO, 60.0f);
		break;
	case SPACESHIP_CAMERA:
		// 플레이어의 특성을 스페이스-쉽 카메라 모드에 맞게 변경(중력은 적용X)
		SetFriction(125.0f);
		...
		break;
	case THIRD_PERSON_CAMERA:
		// 플레이어의 특성을 3인칭 카메라 모드에 맞게 변경(지연 효과와 카메라 오프셋을 설정)
		SetFriction(250.0f);
		SetPGrav = XMFLOAT3(0.0f, -250.0f, 0.0f);
		SetGravity(SetPGrav);
		SetMaxVelocityXZ(300.0f);
